...
const ART = {
  ...
  bnbIcon: "/art/bnb.png",
  quest: "/art/questcomplete.png", // ajusta si tu archivo se llama diferente
  ...
  async function readNft() {
    ...
    try {
      const balance: bigint = await publicClient.readContract({
        address: NFT_ADDRESS, abi: ZETAQUEST_NFT_ABI, functionName: "balanceOf", args: [address],
      });
      if (balance > 0n) {
        const tokenId: bigint = await publicClient.readContract({
          address: NFT_ADDRESS, abi: ZETAQUEST_NFT_ABI, functionName: "tokenOfOwnerByIndex", args: [address, 0n],
        });
        const uri: string = await publicClient.readContract({
          address: NFT_ADDRESS, abi: ZETAQUEST_NFT_ABI, functionName: "tokenURI", args: [tokenId],
        });
        ...
        setNftImage(null); setNftRarity(null);
      }
    } catch {}
  }
  useEffect(() => { readNft(); }, [publicClient, address]);

  /* ------- Guard: si NO tiene NFT, /mint (RPC Zeta fijo) ------- */
  useEffect(() => {
    (async () => {
        ...
        <HoloCard className="md:col-span-1">
          <div className="p-4">
            ...
            <div className="mt-4 grid grid-cols-3 gap-2 text-center">
              <div><div className="text-[10px] opacity-70">Power</div><div className="font-pixel text-xl">{stats.power}</div></div>
              <div><div className="text-[10px] opacity-70">Defense</div><div className="font-pixel text-xl">{stats.defense}</div></div>
              <div>
                <div className="text-[10px] opacity-70">ZetaPoints</div>
            ...
            <div className="mt-4 flex items-center gap-2">
              ...
              <div className="text-sm">Mundo actual: <span className="font-pixel capitalize">{buffs.label}</span></div>
            </div>
